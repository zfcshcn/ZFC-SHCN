...
class ZFCSHGCNCNN(nn.Module):
    def __init__(self, dim_in, dim_out, window_len, link_len, embed_dim, node_num, edge_num):
        ...
        if (dim_in - 3) % 16 == 0:  # (dim_in-3)%16 ==0
            self.window_weights_temporal = nn.Parameter(torch.FloatTensor(embed_dim, 3, int(dim_out / 4)))
        else:  # dim_in == dim_out
            self.window_weights_temporal = nn.Parameter(torch.FloatTensor(embed_dim, int(dim_in / 2), int(dim_out / 4)))
